...
<div class="container-fluid">
    ...
    <div class="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-4">
        <div>
            ...
            <p class="text-muted mb-0">Comprehensive active member </p>
        </div>
        ...
        <div class="card-header" style="background: linear-gradient(135deg, #17479E 0%, #4facfe 100%); border: none; padding: 1.25rem 1.5rem;">
            <div class="d-flex justify-content-between align-items-center">
                <h5 class="mb-0 text-white fw-bold">
                    <i class="fas fa-table me-2"></i>Members 
                </h5>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="align-middle px-4">
                                <span class="badge bg-primary text-white px-3 py-2" style="">{{ $member->checkNumber }}</span>
                            </td>
                            <td class="align-middle px-4">
                                <span class="text-muted">{{ $member->nationalId }}</span>
                            </td>
                            <td class="align-middle px-4">
                                <strong class="text-muted">{{ $member->firstName }} {{ $member->middleName }} {{ $member->lastName }}</strong>
                            </td>
                            <td class="align-middle px-4">
                            ...
                            <td class="align-middle px-4">
                                <span class="badge bg-secondary   px-3 py-2">{{ $member->voteCode }}</span>
                            </td>
                            <td class="align-middle px-4">
                                <span class="badge text-secondary rounded-pill px-3 py-2">{{ $member->deptName }}</span>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
        ...
        <div class="card-footer bg-white border-0">
            <div class="d-flex justify-content-between align-items-center">
                <div class="text-muted small">
                    Showing {{ $members->firstItem() }} to {{ $members->lastItem() }} of {{ number_format($members->total()) }} results
                </div>
                <div>
                ...
                <div class="modal-body p-4">
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <input type="date" name="date" class="form-control rounded-pill" value="{{ request('date', $date) }}">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label fw-semibold">
                                <i class="fas fa-hashtag me-1 text-info"></i>Check Number
                            </label>
                            <input type="text" name="checkNumber" class="form-control rounded-pill" placeholder="Enter check number" value="{{ request('checkNumber', $checkNumber) }}">
                        </div>
                        <div class="col-md-4">
                            <label class="form-label fw-semibold">
                                <i class="fas fa-user me-1 text-success"></i>First Name
                            </label>
                            <input type="text" name="firstName" class="form-control rounded-pill" placeholder="Enter first name" value="{{ request('firstName', $firstName) }}">
                        </div>
                        <div class="col-md-4">
                            <label class="form-label fw-semibold">
                                <i class="fas fa-user me-1 text-success"></i>Middle Name
                            </label>
                            <input type="text" name="middleName" class="form-control rounded-pill" placeholder="Enter middle name" value="{{ request('middleName', $middleName) }}">
                        </div>
                        <div class="col-md-4">
                            <label class="form-label fw-semibold">
                                <i class="fas fa-user me-1 text-success"></i>Last Name
                            </label>
                            <input type="text" name="lastName" class="form-control rounded-pill" placeholder="Enter last name" value="{{ request('lastName', $lastName) }}">
                        </div>
                        <div class="col-md-12">
                            <label class="form-label fw-semibold">
