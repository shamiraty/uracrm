...
class EmployeeLoanController extends Controller
{
    ...
    private function handleLoanChargesRequest($xml)
    {
        ...
        $P = $requestedAmount; // If provided, P is the max net loan amount and should not be recalculated
        ...
        // Variables to track which scenario we're in:
        ...
        if ($scenarioPN) {
            ...
            if ($N > $maxTenureAllowed) {
                $N = $maxTenureAllowed;
                // P & M given. We do not recalculate P since requested amount must remain as is.
                // M also remains as is since it was given.
            }
        } elseif ($scenarioMN) {
            // M & N => Solve P
            $N = $tenure; // Already clamped
            $powResult = pow(1 + $r, $N);
            $num = $powResult - 1;
            $den = $r * $powResult;
            if ($den == 0) {
            ...
            $P = ($M * $num) / $den;
            // If we adjust N later, we can recalc P since requested amount was not given (P=0 originally).
        } else {
        ...
        // If after scenario solving N > maxTenureAllowed, clamp again and recalc only if P was not requested
        if ($N > $maxTenureAllowed) {
            $N = $maxTenureAllowed;
            // Recalculate only if scenario requires and requestedAmount=0:
            if ($scenarioPN) {
                // P & N => M was calculated above, recalc M since N changed
                // M is derived from P & N, requestedAmount given means P fixed.
                $powResult = pow(1 + $r, $N);
                $numerator = $r * $powResult;
                $denominator = $powResult - 1;
                $M = $P * ($numerator / $denominator);
            } elseif ($scenarioMN && $requestedAmount == 0) {
                // M & N => P scenario and requestedAmount=0 means we can recalc P
                $powResult = pow(1 + $r, $N);
                $num = $powResult - 1;
                $den = $r * $powResult;
                $P = ($M * $num) / $den;
            }
            // If scenarioPM, no recalc since P & M are given and P is from requested amount.
        }

        // No recalculation of requestedAmount (P) if it was given as requested amount.
        // The above logic ensures we only recalc P if requestedAmount=0.
