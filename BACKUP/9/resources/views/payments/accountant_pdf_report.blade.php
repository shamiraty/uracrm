...
<head>
    <meta charset="utf-8">
    <title>Accountant Payment Report - {{ strtoupper($accountant->name) }}</title>
    <style>
        @page {
            margin: 80px 40px;
        }
...
<body>
    <!-- Header -->
    <div class="header">
        <h1>Accountant Payment Report</h1>
    ...
    <div class="info-box">
        <p><strong>ACCOUNTANT:</strong> {{ strtoupper($accountant->name) }}</p>
        <p><strong>EMAIL:</strong> {{ strtoupper($accountant->email) }}</p>
        @if($enquiryType)
        <p><strong>ENQUIRY TYPE:</strong> {{ strtoupper(str_replace('_', ' ', $enquiryType)) }}</p>
        @endif
    ...
    <p style="margin-bottom: 10px; font-size: 9pt; color: #666;">
        ...
        <strong>Accountant:</strong> {{ strtoupper($accountant->name) }}
    </p>
