                            ...
                            <div class="current-file-info" style="background: white; border-radius: 8px; padding: 1rem; margin-bottom: 1rem; border: 1px solid rgba(23, 71, 158, 0.1);">
                                <div class="d-flex align-items-center justify-content-between">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div>
                                            <div class="fw-bold" style="color: #1e293b;">{{ basename($folio->file_path) }}</div>
                                            <small class="text-muted d-block">Uploaded: {{ $folio->created_at->format('M d, Y H:i') }}</small>
                                            <small class="text-muted d-block">
                                                @if(file_exists(public_path($folio->file_path)))
                                                    <span class="me-2"><i class="fas fa-hdd me-1" style="color: #17479e;"></i>{{ number_format(filesize(public_path($folio->file_path)) / 1024 / 1024, 2) }} MB</span>
                                                    <span><i class="fas fa-file-alt me-1" style="color: #28a745;"></i>{{ strtoupper(pathinfo($folio->file_path, PATHINFO_EXTENSION)) }} Document</span>
                                                @else
                                    ...
                                    <div class="file-actions">
                                        <a href="{{ asset($folio->file_path) }}" target="_blank" class="btn btn-sm" style="background: #17479e; color: white; margin-right: 0.5rem;">
                                            <i class="fas fa-eye me-1"></i>View
                                        </a>
                                        <a href="{{ asset($folio->file_path) }}" download class="btn btn-sm btn-success">
                            ...
                            <div class="document-preview-section" style="background: white; border-radius: 8px; padding: 1rem; border: 1px solid rgba(23, 71, 158, 0.1);">
                                ...
                                <div class="pdf-preview-frame">
                                    @if(strtolower(pathinfo($folio->file_path, PATHINFO_EXTENSION)) === 'pdf')
                                        <iframe src="{{ asset($folio->file_path) }}" width="100%" height="400" style="border: 1px solid #dee2e6; border-radius: 8px;" title="Previously uploaded document: {{ basename($folio->file_path) }}"></iframe>
                                    @else
                                        <div class="text-center p-4">
                                            <i class="fas fa-file fa-3x mb-3 text-muted"></i>
                                            <p class="text-muted">Preview not available for this file type</p>
                                            <a href="{{ asset($folio->file_path) }}" target="_blank" class="btn btn-sm btn-primary">
