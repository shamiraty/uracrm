...
class UserController extends Controller
{
    ...
    /**
     * Helper method to apply date range filter (FIXED: No Carbon mutation)
     */
        ...
        switch ($dateRange) {
            ...
            case 'jan_to_june':
                $query->whereBetween('created_at', [
                    \Carbon\Carbon::now()->year . '-01-01 00:00:00',
                    \Carbon\Carbon::now()->year . '-06-30 23:59:59'
                ]);
                break;

            case 'july_to_dec':
                $query->whereBetween('created_at', [
                    \Carbon\Carbon::now()->year . '-07-01 00:00:00',
                    \Carbon\Carbon::now()->year . '-12-31 23:59:59'
                ]);
                break;
    ...
    public function generatePDF(Request $request, $id)
    {
        ...
        // Build query with filters - Use helper method to avoid duplication
        $enquiriesQuery = \App\Models\Enquiry::where('registered_by', $id);
        $this->applyDateRangeFilter($enquiriesQuery, $request->date_range);
        ...
        // Get filtered enquiries for statistics
        ...
        // Last enquiry (from filtered results)
        $lastEnquiry = $enquiries->sortByDesc('created_at')->first();

        // ALL filtered enquiries with relationships
        $allEnquiries = \App\Models\Enquiry::where('registered_by', $id)
            ->when($request->filled('date_range'), function($q) use ($request) {
                $this->applyDateRangeFilter($q, $request->date_range);
            })
