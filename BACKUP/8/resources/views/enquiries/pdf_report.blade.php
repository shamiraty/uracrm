...
<body>
    ...
    <!-- Report Information -->
    <div class="info-box">
        <p><strong>REPORT GENERATED BY:</strong> {{ strtoupper($currentUser->name) }} ({{ $currentUser->email }})</p>
        <p><strong>USER ROLE:</strong> {{ strtoupper($currentUser->getRoleNames()->first() ?? 'N/A') }}</p>
        <p><strong>REPORT DATE:</strong> {{ strtoupper(date('d F Y, H:i:s')) }}</p>
        @if($type || $status || $search || $dateFrom || $dateTo)
        <p><strong>FILTERS APPLIED:</strong>
            @if($type) TYPE: {{ strtoupper(str_replace('_', ' ', $type)) }} |@endif
            @if($status) STATUS: {{ strtoupper(str_replace('_', ' ', $status)) }} |@endif
            @if($search) SEARCH: {{ strtoupper($search) }} |@endif
            @if($dateFrom) FROM: {{ strtoupper(\Carbon\Carbon::parse($dateFrom)->format('d M Y')) }} |@endif
            @if($dateTo) TO: {{ strtoupper(\Carbon\Carbon::parse($dateTo)->format('d M Y')) }}@endif
        </p>
        @endif
    </div>
    ...
    <div class="analytics-grid">
        ...
        <div class="analytics-item">
            <h3>{{ number_format($analytics['pending_overdue']) }}</h3>
            <p>Overdue</p>
        </div>
    </div>
