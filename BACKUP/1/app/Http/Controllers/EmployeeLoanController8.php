...
class EmployeeLoanController extends Controller
{
    ...
    //     Log::info('Decoded Signature:', ['signature_decoded' => bin2hex($signatureDecoded)]);
    ...
    //         if ($verification === 1) {
    //             Log::info("Signature verified successfully with C14N(exclusive={$exclusive}, comments={$withComments})");
    //             openssl_free_key($publicKey);
    //             return true;
    //         } elseif ($verification === 0) {
    //             Log::warning("Signature verification failed with C14N(exclusive={$exclusive}, comments={$withComments})");
    //         } else {
    //             Log::error("Error during signature verification with C14N(exclusive={$exclusive}, comments={$withComments}):", ['error' => openssl_error_string()]);
    //         }
    //     }
    
    //     openssl_free_key($publicKey);
    //     Log::error('Signature verification failed after trying all canonicalization modes');
    //     return false;
    // }
    private function verifySignature($xmlContent, $signature)
    {
