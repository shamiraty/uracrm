...
class EmployeeLoanController extends Controller
{
     ...
     */
    private function sendLoanSMS($phone, $message)
    {
        // Handle null or empty phone
        if (empty($phone)) {
            Log::warning('No phone number provided for SMS', ['message' => $message]);
            return false;
        }
        ...
        if (substr($phone, 0, 1) === '0') {
            $phone = '255' . substr($phone, 1); // Tanzania country code
        }
        
        // Ensure it starts with 255 if it doesn't already
        if (substr($phone, 0, 3) !== '255') {
            // If it's a 9-digit number, assume it's missing the country code
            if (strlen($phone) === 9) {
                $phone = '255' . $phone;
            }
        }
        ...
        $apiKey = 'xYz123#';  // Use the same API key as EnquiryController
        
        $client = new \GuzzleHttp\Client();
        try {
            $response = $client->request('POST', $url, [
                'verify' => false,  // Keep SSL verification disabled as in EnquiryController
                'form_params' => [
                    'msisdn' => $phone,
                    'message' => $message,
                    'key' => $apiKey,
            ...
            Log::info("Loan SMS sent successfully", [
                'phone' => $phone,
                'message' => $message,
            ...
            Log::error('Failed to send loan SMS', [
                'phone' => $phone,
                'message' => $message,
                'error' => $e->getMessage()
            ]);
...
/**************************************************************
    ...
    private function handleLoanFinalApprovalNotification($xml) {
        ...
        $loanOffer->reason = mb_substr($reason, 0, 150);
        ...
        if (strtoupper($approval) === 'APPROVED') {
            ...
            try {
                $phone = $loanOffer->mobile_number ?? $loanOffer->phone_number ?? null;
                if ($phone) {
                    $employeeName = trim(($loanOffer->first_name ?? '') . ' ' . ($loanOffer->last_name ?? ''));
                    $amount = number_format($loanOffer->requested_amount ?? 0, 0, '.', ',');
                    
                    $message = "Dear {$employeeName}, your loan application of TZS {$amount} has been APPROVED and will be disbursed soon. Reference: {$loanOffer->application_number}. URASACCOS";
                    
                    $this->sendLoanSMS($phone, $message);
                }
            } catch (\Exception $e) {
                Log::error('Failed to send approval SMS', ['error' => $e->getMessage()]);
            }
...
public function indexLoanOffers(Request $request)
{
    ...
    if ($request->has('loan_type') && $request->input('loan_type')) {
        ...
        if ($loanType === 'new') {
            $query->where(function($q) {
                $q->where('loan_type', 'new')
                  ->orWhereNull('loan_type')
                  ->orWhere('loan_type', '');
            });
        } elseif ($loanType === 'topup') {
            $query->where('loan_type', 'topup');
        }
    }
    ...
    // Add date-based and loan type counts
    $today = Carbon::today();
    ...
    Log::info('Date ranges for filter counts', [
    ...
    $todayCount = LoanOffer::whereDate('created_at', $today)->count();
    $thisWeekCount = LoanOffer::whereDate('created_at', '>=', $startOfWeek->toDateString())
                              ->whereDate('created_at', '<=', $endOfWeek->toDateString())
                              ->count();
    $thisMonthCount = LoanOffer::whereDate('created_at', '>=', $startOfMonth->toDateString())
                               ->whereDate('created_at', '<=', $endOfMonth->toDateString())
                               ->count();
    
    // Log the counts for debugging
    Log::info('Filter counts', [
        'today' => $todayCount,
        'this_week' => $thisWeekCount,
        'this_month' => $thisMonthCount
    ]);
    
    $newLoanCount = LoanOffer::where(function($q) {
        $q->where('loan_type', 'new')
          ->orWhereNull('loan_type')
          ->orWhere('loan_type', '');
    })->count();
    $topupLoanCount = LoanOffer::where('loan_type', 'topup')->count();
  ...
  public function updateLoanOffer(Request $request, $id)
    {
        ...
        if ($request->input('status') === 'SUBMITTED_FOR_DISBURSEMENT') {
            ...
            if (isset($nmbResponse['body']['payload']['RespStatus']) && $nmbResponse['body']['payload']['RespStatus'] === 'Success') {
                ...
                // Truncate reason to 150 chars for API compliance
                $loanOffer->reason = mb_substr($errorMessage, 0, 150);
                $loanOffer->save();
            ...
            $loanOffer->approval = $request->input('approval', $loanOffer->approval);
            // Truncate reason to 150 chars for API compliance
            $reasonInput = $request->input('reason', $loanOffer->reason);
            $loanOffer->reason = $reasonInput ? mb_substr($reasonInput, 0, 150) : $loanOffer->reason;
            $loanOffer->other_charges = $request->input('other_charges', $loanOffer->other_charges);
            $loanOffer->total_amount_to_pay = $request->input('total_amount_to_pay', $loanOffer->total_amount_to_pay);
            ...
            if ($request->has('approval')) {
                ...
                if ($approvalValue === 'APPROVED') {
                    ...
                    try {
                        ...
                        if ($phone) {
                            $employeeName = trim(($loanOffer->first_name ?? '') . ' ' . ($loanOffer->last_name ?? ''));
                            $amount = number_format($loanOffer->take_home_amount ?? $loanOffer->net_loan_amount ?? $loanOffer->requested_amount ?? 0, 0, '.', ',');
                    ...
                    try {
                        ...
                        if ($phone) {
                            $employeeName = trim(($loanOffer->first_name ?? '') . ' ' . ($loanOffer->last_name ?? ''));
                            $rejectionReason = $request->input('reason') ?: 'Does not meet loan requirements';
                            
                            $message = "Dear {$employeeName}, your loan application has been REJECTED. Reason: {$rejectionReason}. For assistance, contact 0677 026301. URASACCOS";
                            
                            $this->sendLoanSMS($phone, $message);
                        }
                    } catch (\Exception $e) {
                        Log::error('Failed to send rejection SMS', ['error' => $e->getMessage()]);
                    }
                    ...
                    // Send SMS notification for loan cancellation
                    try {
                        $phone = $loanOffer->mobile_number ?? $loanOffer->phone_number ?? null;
                        if ($phone) {
                            $employeeName = trim(($loanOffer->first_name ?? '') . ' ' . ($loanOffer->last_name ?? ''));
                            
                            $message = "Dear {$employeeName}, your loan application has been CANCELLED as per request. Reference: {$loanOffer->application_number}. URASACCOS";
                            
                            $this->sendLoanSMS($phone, $message);
                        }
                    } catch (\Exception $e) {
                        Log::error('Failed to send cancellation SMS', ['error' => $e->getMessage()]);
                    }
                } else {
                    // Save for other cases if needed
                    $loanOffer->save();
                }
        ...
        try {
            ...
            if ($id === null) {
                ...
                foreach ($loanIds as $loanId) {
                    ...
                    $loanOffer->state = 'Disbursement Failure';
                    // Truncate reason to 150 chars for API compliance
                    $loanOffer->reason = mb_substr($detailedMessage, 0, 150);
                    $loanOffer->save();

                    // Create or update LoanOfferApproval for disbursement rejection
                    $approval = LoanOfferApproval::firstOrCreate(
                        [
                            'loan_offer_id' => $loanOffer->id,
                            'approval_type' => 'final'
                        ],
                    ...
                    // Mark as rejected for disbursement
                    $approval->reject(auth()->id(), $detailedMessage);
                    ...
                    // Send SMS notification for disbursement rejection
                    try {
                        $phone = $loanOffer->mobile_number ?? $loanOffer->phone_number ?? null;
                        if ($phone) {
                            $employeeName = trim(($loanOffer->first_name ?? '') . ' ' . ($loanOffer->last_name ?? ''));
                            
                            $message = "Dear {$employeeName}, your loan disbursement has been REJECTED. Reason: {$detailedMessage}. Please contact 0677 026301 for assistance. URASACCOS";
                            
                            $this->sendLoanSMS($phone, $message);
                        }
                    } catch (\Exception $e) {
                        Log::error('Failed to send disbursement rejection SMS', ['error' => $e->getMessage()]);
                    }
            ...
            // Store the customer-friendly reason directly (truncated for API compliance)
            $loanOffer->reason = mb_substr($detailedMessage, 0, 150);
            $loanOffer->save();
    ...
    //         // Create or update LoanOfferApproval for disbursement approval
    //         $approval = LoanOfferApproval::firstOrCreate(
    //             [
    //                 'loan_offer_id' => $loanOffer->id,
    //                 'approval_type' => 'final'
    ...
    //         // Mark as approved for disbursement
    //         $approval->approve(auth()->id(), [
    //             'fsp_reference_number' => 'DISB_' . time(),
    //             'total_amount_to_pay' => $loanOffer->take_home_amount ?? $loanOffer->net_loan_amount ?? $loanOffer->requested_amount
    //         ]);
    ...
    //                 // Send SMS notification for successful disbursement
    //                 try {
    //                     $phone = $loanOffer->mobile_number ?? $loanOffer->phone_number ?? null;
    //                     if ($phone) {
    //                         $employeeName = trim(($loanOffer->first_name ?? '') . ' ' . ($loanOffer->last_name ?? ''));
    //                         $amount = number_format($loanOffer->take_home_amount ?? $loanOffer->net_loan_amount ?? $loanOffer->requested_amount ?? 0, 0, '.', ',');
    //                         $accountNumber = substr($loanOffer->bank_account_number ?? '', -4);
                            
    //                         $message = "Dear {$employeeName}, your loan of TZS {$amount} has been DISBURSED to your account ending ***{$accountNumber}. It will reflect within 24hrs. URASACCOS";
                            
    //                         $this->sendLoanSMS($phone, $message);
    //                     }
    //                 } catch (\Exception $e) {
    //                     Log::error('Failed to send disbursement SMS', ['error' => $e->getMessage()]);
    //                 }
    ...
    //                     $message = "Dear {$employeeName}, your loan of TZS {$amount} has been APPROVED for disbursement to your account ending ***{$accountNumber}. It will reflect within 24hrs. URASACCOS";
    ...
    //         $loanOffer->state = 'Disbursement Failure'; // Use correct state from API doc
    //         // Truncate reason to 150 chars for API compliance
    //         $fullReason = 'System error: ' . $e->getMessage();
    //         $loanOffer->reason = mb_substr($fullReason, 0, 150);
    //         $loanOffer->save();
     ...
     private function processSingleDisbursement($loanOffer)
    {
        try {
            ...
            if (config('services.nmb.enabled', false)) {
                ...
                // Check the primary success flag from NmbService
                if (isset($nmbResponse['success']) && $nmbResponse['success'] === true) {

                    $batchId = $nmbResponse['batch_id'];
                    ...
                    $errorMessage = $nmbResponse['error'] ?? $nmbResponse['message'] ?? 'Unknown error from NMB';
    ...
    if ($firstRecord) {
        ...
        if (strtolower($firstRecord['status']) === 'success') {
            ...
            $loanOffer->update([
                ...
                // Truncate reason to 150 chars for API compliance
                'reason' => mb_substr($failureReason, 0, 150),
            ]);
...
/*========================================================================================
    ...
    private function handleTopUpPayoffBalance($xml)
    {
        ...
        try {
            ...
            $msgDetailsArray = json_decode(json_encode($msgDetails), true);
            Log::info('Incoming Message 11 Details', [
                'sender' => (string)($header->Sender ?? ''),
                'receiver' => (string)($header->Receiver ?? ''),
                'message_details' => $msgDetailsArray
            ]);

            // Extract all fields as per API specification (Message 11)
            $requestData = [
            ...
            if (!$requestData['loan_number']) {
                return $this->generateXmlResponse('8002', 'Missing required field: LoanNumber');
            }

            // Find the loan in database
            $loanOffer = LoanOffer::where('loan_number', $requestData['loan_number'])->first();
            if (!$loanOffer) {
                return $this->generateXmlResponse('8004', 'No existing loan found for LoanNumber: ' . $requestData['loan_number']);
            }

            // Calculate payoff using the correct formula from PDF documentation
            $payoffCalculation = $this->calculateCorrectPayoffAmount($loanOffer, $requestData);

            // Update loan record with calculated values
            $this->updateLoanRecordWithPayoff($loanOffer, $payoffCalculation, $requestData);
            ...
            return $this->generateXmlResponse('9999', 'Internal processing error');
        }
    }

    /**
     * Calculate payoff amount using correct formula from PDF documentation
     * Implements the proper financial formulas for loan amortization
     * 
     * Key formulas:
     * 1. Original Principal: PV = EMI × [1 - (1 + r)^-n] / r
     * 2. Payoff Amount: Payoff = PV × [(1 + r)^n - (1 + r)^m] / [(1 + r)^n - 1]
     * Where:
     ...
     * - m = Payments Made
     */
    private function calculateCorrectPayoffAmount(LoanOffer $loanOffer, array $requestData): array
    {
        Log::info('=== Calculating Payoff Amount using PDF Documentation Formula ===', [
            'loan_number' => $loanOffer->loan_number,
            'deduction_amount' => $requestData['deduction_amount'],
            'deduction_balance' => $requestData['deduction_balance']
        ]);

        // Extract data from ESS request based on PDF documentation
        // IB (Initial Balance): Total amount (principal + interest) originally expected to be paid
        // BA (Balance Amount): Total remaining amount (principal + interest) expected to be paid
        $initialBalance = $requestData['deduction_amount'] ?: (float)$loanOffer->requested_amount;
        $remainingBalance = $requestData['deduction_balance'] ?: (float)$loanOffer->outstanding_balance;

        // EMI (Equated Monthly Installment): The fixed amount paid each month
        $monthlyPayment = (float)($loanOffer->desired_deductible_amount ?: $loanOffer->monthly_payment ?: 0);
        
        // Validate we have necessary data
        if ($monthlyPayment <= 0 || $initialBalance <= 0) {
            throw new \Exception('Invalid loan data: Missing monthly payment or initial balance');
        }

        Log::info('Input Data from ESS', [
            'initial_balance_IB' => $initialBalance,
            'remaining_balance_BA' => $remainingBalance,
            'monthly_payment_EMI' => $monthlyPayment
        ]);

        // n (Total Tenure): The total number of payments for the entire loan term
        // As per PDF, we should determine this from loan's original terms, not estimate
        $tenure = (int)$loanOffer->tenure;
        if ($tenure <= 0) {
            // If not available, derive from IB and EMI (approximate method)
            // Note: This is less accurate than having the actual tenure
            $tenure = round($initialBalance / $monthlyPayment);
            Log::warning('Tenure not available, estimated from IB/EMI', ['estimated_tenure' => $tenure]);
        }

        // r (Monthly Interest Rate): Annual rate / 12
        // Using 12% annual rate as per PDF example
        $annualRate = 0.12; // 12% annual interest rate
        $monthlyRate = $annualRate / 12; // r = 0.01 (1% monthly)

        // STEP 1: Determine Original Principal (PV) using PDF formula
        // Formula: PV = EMI × [1 - (1 + r)^-n] / r
        $originalPrincipal = $this->calculateOriginalPrincipal($monthlyPayment, $monthlyRate, $tenure);

        Log::info('Step 1: Original Principal (PV) Calculated', [
            'total_tenure_n' => $tenure,
            'monthly_rate_r' => $monthlyRate,
            'original_principal_PV' => $originalPrincipal,
            'formula_used' => 'PV = EMI × [1 - (1 + r)^-n] / r'
        ]);

        // STEP 2: Determine Number of Payments Made (m)
        // Critical correction from PDF: Use BA to infer m
        // BA represents sum of remaining EMI payments
        // Remaining payments = BA / EMI
        // Therefore: m = n - (BA / EMI)
        $remainingPayments = round($remainingBalance / $monthlyPayment);
        $paymentsMade = $tenure - $remainingPayments;
        
        // Validate payments made
        if ($paymentsMade < 0) {
            Log::warning('Calculated negative payments made, setting to 0', [
                'calculated_m' => $paymentsMade,
                'n' => $tenure,
        ...
        Log::info('Step 2: Payments Analysis', [
            'remaining_payments' => $remainingPayments,
            'payments_made_m' => $paymentsMade,
            'calculation' => "m = n - (BA / EMI) = $tenure - $remainingPayments"
        ]);

        // STEP 3: Calculate Outstanding Principal (Payoff Amount)
        // PDF Formula: Payoff = PV × [(1 + r)^n - (1 + r)^m] / [(1 + r)^n - 1]
        $outstandingPrincipal = $this->calculatePayoffPrincipal(
            $originalPrincipal,
            $monthlyRate,
            $tenure,
            $paymentsMade
        ...
        Log::info('Step 3: Outstanding Principal Calculated', [
            'formula' => 'Payoff = PV × [(1 + r)^n - (1 + r)^m] / [(1 + r)^n - 1]',
            'outstanding_principal' => $outstandingPrincipal
        ]);

        // STEP 4: Add Accrued Interest Since Last Payment
        ...
        return [
            ...
            'pro_rated_interest' => $accruedInterest,
            'days_since_payment' => $daysSincePayment,
    ...
    /**
     * Calculate original principal (PV) using formula from PDF documentation
     * Formula: PV = EMI × [1 - (1 + r)^-n] / r
     * 
     * @param float $monthlyPayment EMI (Equated Monthly Installment)
     * @param float $monthlyRate r (monthly interest rate)
     * @param int $tenure n (total number of payments)
     * @return float Original Principal (PV)
     */
    private function calculateOriginalPrincipal(float $monthlyPayment, float $monthlyRate, int $tenure): float
    {
        // Validation
        if ($monthlyPayment <= 0 || $tenure <= 0) {
            Log::warning('Invalid parameters for principal calculation', [
        ...
        // Apply PDF Formula: PV = EMI × [1 - (1 + r)^-n] / r
        $factor = (1 - pow(1 + $monthlyRate, -$tenure)) / $monthlyRate;
        $presentValue = $monthlyPayment * $factor;

        Log::info('Original Principal (PV) Calculation', [
            'formula' => 'PV = EMI × [1 - (1 + r)^-n] / r',
            'EMI' => $monthlyPayment,
            'r' => $monthlyRate,
            'n' => $tenure,
            'PV' => round($presentValue, 2)
        ]);
    ...
    /**
     * Calculate outstanding principal (payoff amount) using PDF formula
     * Formula: Payoff = PV × [(1 + r)^n - (1 + r)^m] / [(1 + r)^n - 1]
     * 
     * @param float $originalPrincipal PV (Original Principal)
     * @param float $monthlyRate r (monthly interest rate)
     * @param int $totalTenure n (total number of payments)
     * @param int $paymentsMade m (number of payments already made)
     * @return float Outstanding Principal (Payoff Amount)
     */
    private function calculatePayoffPrincipal(float $originalPrincipal, float $monthlyRate, int $totalTenure, int $paymentsMade): float
    {
        // If no payments made, full principal is outstanding
        if ($paymentsMade <= 0) {
            return $originalPrincipal;
        }

        // If all payments made, loan is paid off
        ...
        // Apply PDF Formula: Payoff = PV × [(1 + r)^n - (1 + r)^m] / [(1 + r)^n - 1]
        ...
        Log::info('PDF-Based Payoff Calculation', [
            'formula' => 'Payoff = PV × [(1 + r)^n - (1 + r)^m] / [(1 + r)^n - 1]',
            'PV' => $originalPrincipal,
            'r' => $monthlyRate,
            'n' => $totalTenure,
            'm' => $paymentsMade,
            '(1+r)^n' => $onePlusR_n,
    ...
    private function calculateInstallmentsPaid($initialBalance, $currentBalance, $monthlyPayment): int
    {
        ...
        // Exactly as specified in the PDF formula
        $paymentsMade = ($initialBalance - $currentBalance) / $monthlyPayment;

        // Round to nearest integer for number of payments
        $estimatedPayments = round($paymentsMade);

        return max(0, $estimatedPayments);
    }

    /**
     * Calculate outstanding principal after payments using amortization schedule
     * This simulates the loan amortization to find the true outstanding principal
     */
    private function calculateOutstandingPrincipal($originalPrincipal, $monthlyPayment, $installmentsPaid, $monthlyRate): float
    {
        ...
        Log::info('Calculating Outstanding Principal via Amortization', [
            'original_principal' => $originalPrincipal,
            'monthly_payment' => $monthlyPayment,
            'installments_to_process' => $installmentsPaid,
            'monthly_rate' => $monthlyRate
        ]);

        // Simulate each payment to get the true outstanding balance
        for ($i = 1; $i <= $installmentsPaid; $i++) {
            // Interest for this month = Outstanding Balance × Monthly Rate
            ...
            // Log every 10th payment for debugging
            if ($i % 10 == 0 || $i == $installmentsPaid) {
                Log::debug("Payment $i processed", [
                    'interest' => round($interestForMonth, 2),
            ...
            if ($balance <= 0) {
                Log::info('Loan fully amortized', ['payments_made' => $i]);
                return 0;
            }
        }
        ...
        Log::info('Outstanding Principal After Amortization', [
            'payments_processed' => $installmentsPaid,
            'final_outstanding_principal' => $finalBalance
        ]);
    ...
    /**
     * Update loan record with calculated payoff values
     */
    private function updateLoanRecordWithPayoff(LoanOffer $loanOffer, array $calculation, array $requestData): void
    {
        ...
        $loanOffer->check_number = $requestData['check_number'];
        $loanOffer->first_name = $requestData['first_name'];
        $loanOffer->middle_name = $requestData['middle_name'];
        $loanOffer->last_name = $requestData['last_name'];
        $loanOffer->vote_code = $requestData['vote_code'];
        $loanOffer->vote_name = $requestData['vote_name'];

        $loanOffer->save();

        Log::info('Loan record updated with payoff calculation', [
            'loan_number' => $loanOffer->loan_number,
            'settlement_amount' => $loanOffer->settlement_amount,
            'outstanding_balance' => $loanOffer->outstanding_balance
        ]);
    }
    ...
    private function getDaysSinceLastPayment(LoanOffer $loanOffer): int
    {
        ...
        return 7; // Default 7 days for weekly processing
    }
...

{
  ...
  $contractEndDate = (string)$msgDetails->ContractEndDate === ''
    ...
    // Calculate take_home_amount and net_loan_amount for top-up
    $netLoanAmount = null;
    $takeHomeAmount = null;
    if ($requestedAmount > 0) {
        // For top-up: net_loan_amount = requested_amount - settlement_amount
        $netLoanAmount = $requestedAmount - $settlementAmount;
        
        // Calculate total charges (processing fee + insurance)
        $totalCharges = $processingFee + $insurance;
        
        // Take home amount = net_loan_amount - total_charges
        $takeHomeAmount = $netLoanAmount - $totalCharges;
        
        // Ensure take_home_amount is not negative
        if ($takeHomeAmount < 0) {
            $takeHomeAmount = 0;
        }
        
        Log::info('TOP_UP_OFFER_REQUEST: Calculated amounts', [
    ...
    $loanOffer = DB::transaction(function () use ($checkNumber, $firstName, $middleName, $lastName, $sex, $bankAccountNumber,
        ...
        $contractStartDate, $contractEndDate, $loanNumber, $settlementAmount, $swiftCode, $funding, $netLoanAmount, $takeHomeAmount) {
        ...
        $loanOffer = LoanOffer::create([
            ...
            'funding'            => $funding,
            'net_loan_amount'    => $netLoanAmount,    // Add calculated net loan amount
            'take_home_amount'   => $takeHomeAmount,   // Add calculated take home amount
            // Additional fields
...
// response generator for the loan liquidation response:
    ...
    public function getFilterCounts()
    {
        ...
        $counts = [
            // Date-based counts
            'today' => LoanOffer::whereDate('created_at', $today)->count(),
            'this_week' => LoanOffer::whereDate('created_at', '>=', $startOfWeek->toDateString())
                                   ->whereDate('created_at', '<=', $endOfWeek->toDateString())
                                   ->count(),
            'this_month' => LoanOffer::whereDate('created_at', '>=', $startOfMonth->toDateString())
                                    ->whereDate('created_at', '<=', $endOfMonth->toDateString())
                                    ->count(),
            
            // Loan type counts
            'new_loans' => LoanOffer::where(function($q) {
                $q->where('loan_type', 'new')
                  ->orWhereNull('loan_type')
                  ->orWhere('loan_type', '');
            })->count(),
            'topup_loans' => LoanOffer::where('loan_type', 'topup')->count(),
            
            // Status counts (these are likely already being passed to the view)
            'pending' => LoanOffer::where(function($q) {
                $q->where('approval', 'PENDING')
                  ->orWhereNull('approval');
            })->count(),
            'approved' => LoanOffer::where('approval', 'APPROVED')->count(),
            'rejected' => LoanOffer::where('approval', 'REJECTED')->count(),
            'cancelled' => LoanOffer::where('approval', 'CANCELLED')->count(),
            'disbursed' => LoanOffer::where('status', 'disbursed')->count(),
        ];
        ...
        try {
            foreach ($loanIds as $loanId) {
                ...
                $disbursement = LoanDisbursement::create([
                    ...
                    'disbursed_at' => now(),
                    'notes' => $request->notes,
                    'account_number' => $loan->bank_account_number ?? '0000000000'
                ]);
