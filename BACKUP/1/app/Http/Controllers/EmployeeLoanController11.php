...
class EmployeeLoanController extends Controller
{
    public function handleRequest(Request $request)
    {
        ...
        // Extract the signature without altering the original XML
        $signature = $this->extractSignature($xmlContent);
        if (!$signature) {
            return $this->generateXmlResponse('8009', 'Invalid Signature');
        }

        Log::info('Extracted Signature: ' . $signature); // Log as a simple string to avoid escaping

        // Verify the signature directly on the raw XML
        if (!$this->verifySignature($xmlContent, $signature)) {
            return $this->generateXmlResponse('8009', 'Invalid Signature');
        }

        // If verification is successful, parse the XML and dispatch
    ...
    private function extractSignature($xmlContent)
    {
        try {
            ...
            Log::error('Error extracting signature: ' . $e->getMessage());
            return null;
        }
    }
    ...
    //     // Canonicalize without altering any attributes, especially for null fields
    //     $dataElementCanonicalized = $dataNode->C14N(true, true); // Try exclusive no comments
    //     Log::info("Canonicalized Data: " . $dataElementCanonicalized);
    ...
    private function verifySignature($xmlContent, $signature)
{
    ...
    // Log available digest methods
    $md_methods = openssl_get_md_methods();
    Log::info("Available Message Digest Methods: " . print_r($md_methods, true));
    ...
    // Remove the xsi namespace attributes from ContractStartDate and ContractEndDate
    ...
    if ($contractStartDate && $contractStartDate->hasAttributeNS('http://www.w3.org/2000/xmlns/', 'xmlns:xsi')) {
        $contractStartDate->removeAttributeNS('http://www.w3.org/2000/xmlns/', 'xmlns:xsi');
    }
    if ($contractEndDate && $contractEndDate->hasAttributeNS('http://www.w3.org/2000/xmlns/', 'xmlns:xsi')) {
        $contractEndDate->removeAttributeNS('http://www.w3.org/2000/xmlns/', 'xmlns:xsi');
    }

    // Also, since they are empty, remove those attributes entirely
    if ($contractStartDate && $contractStartDate->hasAttributeNS('http://www.w3.org/2001/XMLSchema-instance', 'nil')) {
        $contractStartDate->removeAttributeNS('http://www.w3.org/2001/XMLSchema-instance', 'nil');
    }
    if ($contractEndDate && $contractEndDate->hasAttributeNS('http://www.w3.org/2001/XMLSchema-instance', 'nil')) {
        $contractEndDate->removeAttributeNS('http://www.w3.org/2001/XMLSchema-instance', 'nil');
    }
    ...
    // Assume SHA256 for now, but get the correct algorithm from the sender.
    $opensslAlgorithm = OPENSSL_ALGO_SHA256;
    Log::info("Using OpenSSL algorithm: " . $opensslAlgorithm);

    // Canonicalization without comments
    $dataElementCanonicalized = $dom->C14N(false, false, null, null);
    // Remove <Document> and </Document> that are added
    $dataElementCanonicalized = str_replace("<Document>", "", $dataElementCanonicalized);
    $dataElementCanonicalized = str_replace("</Document>", "", $dataElementCanonicalized);
    Log::info("Canonicalized Data (No Exclusive, No Comments, No Document tags, No xsi): " . $dataElementCanonicalized);
    Log::info("Canonicalized Data (Hex): " . bin2hex($dataElementCanonicalized));

    $verification = openssl_verify($dataElementCanonicalized, $signatureDecoded, $publicKey, $opensslAlgorithm);
    openssl_free_key($publicKey);
