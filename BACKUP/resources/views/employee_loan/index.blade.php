            ...
            <form method="GET" action="{{ route('loan-offers.index') }}" id="filter-form">
                ...
                <div class="filter-pills mb-3">
                    ...
                    <div class="filter-pills-group">
                        <!-- Loan type filters -->
                        <button type="button" class="filter-pill filter-pill-new" onclick="applyQuickFilter('new_loan')" data-filter="new_loan">
                            <i class="fas fa-plus-circle me-1"></i>New Loan
                            <span class="filter-pill-count" id="new-loan-count">{{ $newLoanCount ?? 0 }}</span>
                        </button>
                        <button type="button" class="filter-pill filter-pill-topup" onclick="applyQuickFilter('topup')" data-filter="topup">
                            <i class="fas fa-arrow-up-circle me-1"></i>Top-up
                            <span class="filter-pill-count" id="topup-count">{{ $topupLoanCount ?? 0 }}</span>
                        </button>
                        ...
                        <button type="button" class="filter-pill filter-pill-cancelled" onclick="applyQuickFilter('cancelled')" data-filter="cancelled">
                            <i class="fas fa-ban me-1"></i>Cancelled
                            <span class="filter-pill-count">{{ $cancelledCount ?? 0 }}</span>
                        </button>
                        <button type="button" class="filter-pill filter-pill-rejected" onclick="applyQuickFilter('rejected')" data-filter="rejected">
    ...
    <div class="modern-table-wrapper">
        <div class="modern-table-header-section">
            <div class="table-header-content">
                ...
                <!-- Table actions group can be kept for future buttons if needed -->
                            ...
                            <tr class="modern-table-row clickable-row" data-id="{{ $offer->id }}" data-loan='{{ json_encode($loanData) }}'>
                                            ...
                                            <div class="employee-id">
                                                ...
                                                @if($offer->loan_type === 'topup' && $offer->topupAsNew && $offer->topupAsNew->original_loan_number)
                                                    <span class="badge bg-secondary ms-2" style="font-size: 0.6rem;">
                                                        <i class="fas fa-link"></i> {{ $offer->topupAsNew->original_loan_number }}
                                                    </span>
                                                @endif
                                            </div>
                                        </div>
                                ...
                                <td class="text-center">
                                    @switch($offer->approval)
                                        ...
                                        @case('REJECTED')
                                            <div>
                                                <span class="modern-status-badge status-rejected">
                                                    <i class="fas fa-times-circle"></i> Rejected
                                                </span>
                                                @if($offer->reason)
                                                    <div class="reason-display mt-1">
                                                        <small class="text-danger d-block" style="font-size: 0.7rem; line-height: 1.2;">
                                ...
                                <td class="text-center">
                                    ...
                                    @switch(strtolower($state))
                                        ...
                                        @case('fsp rejected')
                                            @if($offer->reason)
                                                <span class="modern-status-badge status-rejected" 
                                                      ...
                                                      data-bs-html="true"
                                                      title="FSP Rejection" 
                                                      data-bs-content="<strong>Reason:</strong><br>{{ e($offer->reason) }}">
                                                    <i class="fas fa-times-circle"></i> FSP Rejected
                                                    <i class="fas fa-info-circle ms-1" style="font-size: 0.75rem;"></i>
                                                </span>
                                            @else
                                        ...
                                        @case('employee cancelled')
                                            ...
                                            @if($offer->reason)
                                                <br>
                                                <small class="text-muted" style="font-size: 0.65rem; line-height: 1.2; display: inline-block; margin-top: 2px;">
                                                    <i class="fas fa-info-circle" style="font-size: 0.6rem;"></i>
                                                    {{ Str::limit($offer->reason, 35) }}
                                                </small>
                                            @endif
                                            @break
                            ...
                            <div class="col-md-12">
                                <div class="info-card-enhanced">
                                    ...
                                    <div class="info-card-body">
                                        ...
                                        <div class="row mt-3" id="modal-reason-section" style="display: none;">
                                            <div class="col-12">
                                                <div class="alert mb-0" id="modal-reason-alert">
                                                    <h6 class="alert-heading" id="modal-reason-title">
                                                        <i class="fas fa-info-circle me-2"></i>Reason
                                                    </h6>
                                                    <p class="mb-0" id="modal-reason-text"></p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
...
.filter-pill-new {
    ...
    background: linear-gradient(135deg, rgba(23, 162, 184, 0.05) 0%, rgba(33, 150, 243, 0.05) 100%);
}

.filter-pill-new:hover {
    background: linear-gradient(135deg, #17a2b8 0%, #2196F3 100%);
    color: white;
    border-color: var(--ura-info);
}
    ...
    if (loanModal) {
        loanModal.addEventListener('show.bs.modal', function (event) {
            ...
            if (button) {
                ...
                // Show rejection/cancellation reason in Status tab if available
                const reasonSection = document.getElementById('modal-reason-section');
                const reasonAlert = document.getElementById('modal-reason-alert');
                const reasonTitle = document.getElementById('modal-reason-title');
                const reasonText = document.getElementById('modal-reason-text');
                
                if (loan.reason && (loan.approval === 'REJECTED' || loan.approval === 'CANCELLED' || 
                    loan.status === 'REJECTED' || loan.status === 'CANCELLED' ||
                    (loan.state && loan.state.toLowerCase().includes('rejected')) ||
                    (loan.state && loan.state.toLowerCase().includes('cancelled')))) {
                    
                    if (reasonSection) reasonSection.style.display = 'block';
                    
                    // Set appropriate alert color and title based on status
                    if (loan.approval === 'REJECTED' || loan.status === 'REJECTED' || 
                        (loan.state && loan.state.toLowerCase().includes('rejected'))) {
                        if (reasonAlert) reasonAlert.className = 'alert alert-danger mb-0';
                        if (reasonTitle) reasonTitle.innerHTML = '<i class="fas fa-times-circle me-2"></i>Rejection Reason';
                    } else {
                        if (reasonAlert) reasonAlert.className = 'alert alert-warning mb-0';
                        if (reasonTitle) reasonTitle.innerHTML = '<i class="fas fa-ban me-2"></i>Cancellation Reason';
                    }
                    
                    if (reasonText) reasonText.textContent = loan.reason;
                } else {
                ...
                // Check if loan is cancelled, rejected, or approved and update Process Loan tab accordingly
                ...
                const isRejected = loan.approval === 'REJECTED' || loan.status === 'REJECTED' || 
                                  (loan.state && loan.state.toLowerCase().includes('rejected'));
                
                const isApproved = loan.approval === 'APPROVED' || loan.status === 'disbursed' ||
                                  (loan.state && loan.state.toLowerCase().includes('approved'));
                ...
                if (isCancelled) {
                    ...
                    if (cancelledWarning) {
                        ...
                        if (loan.reason) {
                            const reasonDisplay = document.getElementById('cancellation-reason-display');
                            const reasonText = document.getElementById('cancellation-reason-text');
                            if (reasonDisplay) reasonDisplay.style.display = 'block';
                            if (reasonText) reasonText.textContent = loan.reason;
                        }
                    }
                    ...
                    // For rejected loans, show rejection message and disable actions
                    if (statusAlert) {
                        statusAlert.style.display = 'block';
                        statusAlert.className = 'alert alert-danger';
                        statusAlert.innerHTML = '<i class="fas fa-times-circle me-2"></i><strong>Loan Rejected:</strong> This loan has been rejected and no further actions are available.';
                    }
                    ...
                    // For approved loans, show success message and disable actions
                    if (statusAlert) {
                        statusAlert.style.display = 'block';
                        statusAlert.className = 'alert alert-success';
                        statusAlert.innerHTML = '<i class="fas fa-check-circle me-2"></i><strong>Loan Approved:</strong> This loan has been approved. No further approval actions required.';
                    }
                    if (cancelledWarning) cancelledWarning.style.display = 'none';
                    
                    // Disable approval/rejection action cards
                    actionCards.forEach(card => {
                        card.classList.add('disabled');
                        card.style.pointerEvents = 'none';
                        card.style.opacity = '0.5';
        ...
        row.addEventListener('click', function(e) {
            ...
            if (loanDataStr) {
                try {
                    ...
                    // Click the button to trigger Bootstrap's modal properly
                    tempButton.click();
...
function applyQuickFilter(filter) {
    ...
    // Add loan_type input if it doesn't exist
    if (!form.querySelector('[name="loan_type"]')) {
        const loanTypeInput = document.createElement('input');
        loanTypeInput.type = 'hidden';
        loanTypeInput.name = 'loan_type';
        form.appendChild(loanTypeInput);
    }
    form.querySelector('[name="loan_type"]').value = '';
    ...
    switch(filter) {
        // Loan type filters
        case 'new_loan':
            form.querySelector('[name="loan_type"]').value = 'new';
            break;
            
        case 'topup':
            form.querySelector('[name="loan_type"]').value = 'topup';
            break;
        ...
        case 'cancelled':
            form.querySelector('[name="status"]').value = 'cancelled';
            break;
...
// Function to update filter counts (now optional since counts are passed from controller)
function updateFilterCounts() {
    // Counts are now passed directly from the controller
    // This function can still be used if you want to refresh counts via AJAX
    fetch('/api/loan-offers/filter-counts')
        .then(response => response.json())
        .then(data => {
            // Update time-based filter counts
            ...
            // Update loan type counts
            document.getElementById('new-loan-count').textContent = data.new_loans || 0;
            document.getElementById('topup-count').textContent = data.topup_loans || 0;
        })
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // updateFilterCounts(); // Commented out since counts are now passed from controller
    animateKPICards();
