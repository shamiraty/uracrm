...
class SickLeaveExport implements FromCollection, WithHeadings
{
    ...
    public function collection()
    {
        ...
        return $query->get()->map(function ($sickLeave) {
            $enquiry = $sickLeave->enquiry;
            $startdate = Carbon::parse($sickLeave->startdate);
            $enddate = Carbon::parse($sickLeave->enddate);
            $days = $startdate->diffInDays($enddate);
            $status = $days >= 30 ? "Eligible" : "Not Eligible";
